...
def ProjectParameter_create_for_Spark():
    ...
    Final_sheet_data.rename(
        columns={
            ...
            "小区RE参考功率": "RePower(0.1dbm)",
            "运行状态的详细信息(DU小区配置)": "OnAirStatus",
            "业务IP地址": "ServiceIP",
            "站点类型": "Indoor",
        },
        inplace=True,
    ...
    Final_sheet_data["Indoor"].replace(
        {"宏站": False, "室分": False, "微站": False}, inplace=True
    )
    ...
    #    #output the data to txt format files
    ...
    #    real_data.to_csv(os.path.join(current_path,'Spark5G工参_'+date_today+'.txt'),header=True, index=False,sep='\t')
    #
