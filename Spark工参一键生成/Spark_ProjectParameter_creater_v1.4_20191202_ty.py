...
#
if __name__="__main__":
	print("Welcome to use Spark_Pioneer_CNO onkey Project Parameter Create Tool!")
	print("The Process is starting now.......")
	#在当前路径下以日期为名称创建保存文件夹
	try:
		date_today=datetime.datetime.now().strftime("%Y%m%d")
		Save_dir=os.path.join(os.getcwd(),date_today+"工参")
		if not os.path.exists(Save_dir):
            ...
            print("Run Error!")
        Excel_sheet_data_NR,Excel_sheet_data_FDD=(ProjectParameter_read_from_Source_excrel())
        Spark_NR=NR_Data_For_Spark(Exel_sheet_data_NR)
        Export_data_to_excel(Spark_NR,"Spark5G工参","NR_Spark")
        Pioneer_NR=NR_Data_For_Pioeer(Excel_sheet_data_NR)
